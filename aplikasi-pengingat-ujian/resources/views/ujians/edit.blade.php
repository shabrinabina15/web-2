                    ...
                    <form method="POST" action="{{ route('ujian.update', $ujian) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="mata_pelajaran_id" name="mata_pelajaran_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                ...
                                <option value="{{ $mapel->id }}" @if(old('mata_pelajaran_id', $ujian->mata_pelajaran_id) == $mapel->id) selected @endif>
                                    {{ $mapel->nama_mapel }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        {{-- Nama Ujian --}}
                        <div>
                            <x-input-label for="nama_ujian" :value="__('Nama Ujian')" />
                            <x-text-input id="nama_ujian" name="nama_ujian" type="text" class="mt-1 block w-full" :value="old('nama_ujian', $ujian->nama_ujian)" required autofocus />
                        ...
                        <div>
                            ...
                            <x-text-input id="lokasi" name="lokasi" type="text" class="mt-1 block w-full" :value="old('lokasi', $ujian->lokasi)" placeholder="Contoh: R.401, Gedung A" />
                        ...
                        <div>
                            ...
                            <x-text-input id="tanggal_ujian" name="tanggal_ujian" type="datetime-local" class="mt-1 block w-full" :value="old('tanggal_ujian', $ujian->tanggal_ujian->format('Y-m-d\TH:i'))" required />
                        ...
                        <div>
                            ...
                            <textarea id="catatan" name="catatan" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('catatan', $ujian->catatan) }}</textarea>
